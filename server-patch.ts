  ...
  try {
    for (const f of readdirSync(sessionsDir)) {
      ...
      try {
        ...
        if (d.pid && d.kind === 'interactive') return d.pid
      } catch {}
    }
  } catch {}
  return null
...
function getUsageInfo(): string {
  ...
  let rateLimitsText = ''
  try {
    const raw = execSync('claude --strict-mcp-config -p /usage 2>&1', {
      timeout: 10000,
      encoding: 'utf8',
...
function injectCommand(cmd: string, chatId: string, userId: string): void {
  mcp.notification({
    ...
    params: {
      content: cmd,
      meta: { chat_id: chatId, user: userId, user_id: userId, ts: new Date().toISOString() },
    },
  }).catch(() => {})
}
...
bot.on('callback_query:data', async ctx => {
  ...
  if (ctrl) {
    ...
    switch (ctrl[1]) {
      case 'stop': {
        const pid = getClaudePid()
        if (pid) { try { process.kill(pid, 'SIGINT') } catch {} }
        await ctx.answerCallbackQuery({ text: '⏹ Interrupted' }).catch(() => {})
        await ctx.reply('⏹ Interrupted Claude.').catch(() => {})
        break
      }
      case 'compact':
  ...
  const modelBtn = /^model:(.+)$/.exec(data)
  if (modelBtn) {
    const access = loadAccess()
    if (!access.allowFrom.includes(senderId)) {
      await ctx.answerCallbackQuery({ text: 'Not authorized.' }).catch(() => {})
...
bot.command('stop', async ctx => {
  if (!dmCommandGate(ctx)) return
  const pid = getClaudePid()
  if (!pid) { await ctx.reply('No active Claude session found.'); return }
  try { process.kill(pid, 'SIGINT') } catch {}
  await ctx.reply('⏹ Interrupted Claude.')
})
